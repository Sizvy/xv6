  ...
  1d:	89 e1                	mov    %esp,%ecx
{
  ...
  76:	ff 34 b8             	pushl  (%eax,%edi,4)
  79:	e8 32 02 00 00       	call   2b0 <atoi>
    for(i=1;i<argc;i++)
  7e:	83 c4 10             	add    $0x10,%esp
        arr[i]=atoi(argv[i]);
  81:	89 04 be             	mov    %eax,(%esi,%edi,4)
    for(i=1;i<argc;i++)
  84:	83 c7 01             	add    $0x1,%edi
  87:	39 fb                	cmp    %edi,%ebx
  89:	75 e5                	jne    70 <main+0x70>
    }
    printf(1,"Sum is: %d\n",getarraySum(size-1,arr));
  8b:	83 ec 08             	sub    $0x8,%esp
  8e:	56                   	push   %esi
  8f:	ff 75 e0             	pushl  -0x20(%ebp)
  92:	e8 3c 03 00 00       	call   3d3 <getarraySum>
  97:	83 c4 0c             	add    $0xc,%esp
  9a:	50                   	push   %eax
  9b:	68 08 08 00 00       	push   $0x808
  a0:	6a 01                	push   $0x1
  a2:	e8 f9 03 00 00       	call   4a0 <printf>
    exit();
  a7:	e8 77 02 00 00       	call   323 <exit>
    int arr[size-1];
  ac:	83 4c 14 fc 00       	orl    $0x0,-0x4(%esp,%edx,1)
  b1:	eb ac                	jmp    5f <main+0x5f>
  b3:	66 90                	xchg   %ax,%ax
  b5:	66 90                	xchg   %ax,%ax
  b7:	66 90                	xchg   %ax,%ax
  b9:	66 90                	xchg   %ax,%ax
...
000003d3 <getarraySum>:
SYSCALL(getarraySum)
 3d3:	b8 18 00 00 00       	mov    $0x18,%eax
 3d8:	cd 40                	int    $0x40
 3da:	c3                   	ret    

000003db <getreadcount>:
...
printf(int fd, const char *fmt, ...)
{
 ...
 624:	eb 1a                	jmp    640 <printf+0x1a0>
          s = "(null)";
 626:	bb 14 08 00 00       	mov    $0x814,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 675:	a1 d0 0a 00 00       	mov    0xad0,%eax
{
 ...
 696:	73 10                	jae    6a8 <free+0x38>
{
 ...
 6c1:	5b                   	pop    %ebx
  freep = p;
 6c2:	a3 d0 0a 00 00       	mov    %eax,0xad0
}
 6c7:	5e                   	pop    %esi
 6c8:	5f                   	pop    %edi
 6c9:	5d                   	pop    %ebp
 ...
 6e7:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 6ea:	a3 d0 0a 00 00       	mov    %eax,0xad0
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 70d:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 710:	8b 3d d0 0a 00 00    	mov    0xad0,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 759:	8b 3d d0 0a 00 00    	mov    0xad0,%edi
 ...
 7b4:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 7b7:	a3 d0 0a 00 00       	mov    %eax,0xad0
}
 ...
 7ce:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 7d0:	c7 05 d0 0a 00 00 d4 	movl   $0xad4,0xad0
 7d7:	0a 00 00 
    base.s.size = 0;
 7da:	bf d4 0a 00 00       	mov    $0xad4,%edi
    base.s.ptr = freep = prevp = &base;
 7df:	c7 05 d4 0a 00 00 d4 	movl   $0xad4,0xad4
 7e6:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 7e9:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 7eb:	c7 05 d8 0a 00 00 00 	movl   $0x0,0xad8
 7f2:	00 00 00 
    if(p->s.size >= nunits){
